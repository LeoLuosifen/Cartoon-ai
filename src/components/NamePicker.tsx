      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6 items-start">
        ...
        >
          ...
          {names.length === 0 ? (
            ...
            <div 
              ref={listRef}
              className="flex-1 bg-slate-50 rounded-2xl border-2 border-slate-900 p-4 overflow-y-auto max-h-[300px] space-y-2 scroll-smooth"
            >
              {names.map((name, idx) => (
                <div 
                  key={idx} 
                  data-index={idx}
                  className="px-3 py-2 rounded-xl border-2 font-bold transition-all flex items-center gap-2 bg-white text-slate-700 border-slate-200"
                >
          ...
          <div className="mt-6 p-4 bg-purple-50 rounded-2xl border-2 border-dashed border-primary/30 flex items-start gap-3">
          ...
          <AnimatePresence mode="wait">
            {names.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40">
                ...
                <p className="font-bold text-slate-500">上传名单后即可开始点名！</p>
              </div>
            )}
          </AnimatePresence>
        </div>
